...
def test_plot_and_statistics_updates(sensor_plotter):
    ...
    sensor_plotter.full_sensor_data = [data.copy() for data in sensor_plotter.sensor_data]
    ...
    for sensor in sensor_plotter.sensor_config:
        ...
        assert labels is not None, f"No stats labels found for sensor '{sensor['name']}'."
        # In our simulated data, current value should be 50, min 10, max 50, and avg 30.
        assert "50" in labels["current"].text(), f"Expected current value '50' for {sensor['name']}."
        assert "10" in labels["min"].text(), f"Expected min value '10' for {sensor['name']}."
        assert "50" in labels["max"].text(), f"Expected max value '50' for {sensor['name']}."
        assert "30" in labels["avg"].text(), f"Expected avg value '30' for {sensor['name']}."
