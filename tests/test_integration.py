...
from sensor import Sensor, ADC_MAX, VOLTAGE_FULL_SCALE, POLL_INTERVAL_MS
...
def test_modbus_start_and_stop(qtbot, sensor_plotter, monkeypatch):
    ...
    monkeypatch.setattr(sensor_plotter, 'modbus_client', fake_client)
    ...
    assert hasattr(sensor_plotter, 'sensor_worker') and sensor_plotter.sensor_worker is not None, "Expected sensor worker to be started."
    
    # Now, stop the modbus connection.
    sensor_plotter.stop_modbus()
    
    # Verify cleanup.
    assert sensor_plotter.sensor is None, "Expected sensor instance to be None after stopping."
    assert not (hasattr(sensor_plotter, 'sensor_worker') and sensor_plotter.sensor_worker), "Expected sensor worker to be None after stopping."
